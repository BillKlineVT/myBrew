...
export default function ChillTab({ latest, history }: Props) {
  const chartData = history.slice(-120).map((r, i) => ({ t: i, temp: r.chill_temp }))

  return (
    <div style={styles.container}>
      <h2 style={styles.h2}>Chiller</h2>
      <div style={styles.row}>
        <div style={styles.panel}>
          <div style={styles.stat}>
            <span style={styles.statLabel}>Chill Temp</span>
            <span style={styles.statVal}>{latest.chill_temp?.toFixed(1) ?? '—'} °F</span>
          </div>
          <div style={{ marginTop: 8, fontSize: 13, color: '#718096' }}>
            Target pitch temp: ~65–70 °F for ale yeast.
          </div>
        </div>
      </div>
      <div style={styles.chartWrap}>
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={chartData}>
            ...
            <YAxis domain={['auto', 'auto']} stroke="#76e4f7" />
            <Tooltip contentStyle={{ background: '#1a202c', border: '1px solid #4a5568' }} />
            <Legend />
            <Line type="monotone" dataKey="temp" stroke="#76e4f7" dot={false} name="Chill Temp °F" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
