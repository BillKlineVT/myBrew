    ...
    stage('execute-sikulix-auto-gui-test') {
      steps { 
          ...
          withCredentials([usernameColonPassword(credentialsId: 'c603ab1f-f27a-4da2-bea2-f010ee12676a', variable: 'USERPASS')]) {
            ...
            sh 'ssh -t root@192.168.1.5 "cd; rm -f SUT; mkdir -p SUT; cd SUT; tar xzvf ../myBrew_v\$BUILD_NUMBER.tar.gz; export DISPLAY=:1; cd GUITest; mvn clean package; java -jar target/GUITest.myBrewGUITest-0.0.1-SNAPSHOT.jar > ../myBrew_v\$BUILD_NUMBER_auto_gui_test_results.txt"'
