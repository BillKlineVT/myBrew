    ...
    stage('execute-sikulix-auto-gui-test') {
      steps { 
          ...
          withCredentials([usernameColonPassword(credentialsId: 'c603ab1f-f27a-4da2-bea2-f010ee12676a', variable: 'USERPASS')]) {
            ...
            sh 'ssh -t root@192.168.1.5 "cd; mkdir -p SUT; tar xzvf myBrew_v\$BUILD_NUMBER.tar.gz;cd ~/SUT/myBrewTests/GUITest/GUITest; export DISPLAY=:1; java -jar target/GUITest.myBrewGUITest-0.0.1-SNAPSHOT.jar > myBrew_auto_gui_test_results.txt"'
          }
          //sh '''
          //  cd myBrewTests/GUITest/GUITest
          //  export DISPLAY=:1
